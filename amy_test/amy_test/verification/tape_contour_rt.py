...
class TapeContourRT(Node):
    def __init__(self):
        ...
        self.dead_end_y_threshold = 0.7  # Tape must be in bottom 30% of frame (cy > 0.7*height)
        self.probe_forward_duration = 4.0  # Probe forward for 4 seconds looking for tape (increased)
        self.probe_forward_speed = 0.5  # Moderate speed when probing (50% of normal, was 30%)
        self.cautious_frames = 0  # Count frames in cautious approach to dead end
    ...
    def image_callback(self, msg: Image):
        ...
        if not tape_detected:
            ...
            # Phase 3: Advanced recovery (backup, then systematic turn search)
                ...
                if self.recovery_phase == 'NONE':
                    ...
                    # Skip backup if dead end detected - go straight to probe forward
                    elif self.dead_end_detected:
                        self.recovery_phase = 'PROBE_FORWARD'
                        self.recovery_start_time = time.perf_counter()
                        self.get_logger().warn('Dead end recovery: Skipping backup, going to PROBE_FORWARD')
                    else:
                ...
                # Sub-phase 3a: Back up for 1.5 seconds
                if self.recovery_phase == 'BACKING_UP':
                    elapsed = current_time - self.recovery_start_time
                    
                    # Check if we see tape during backup (might be a dead end)
                    if best_contour is not None and contour_angle is not None:
                        # We see tape during backup - check if it's straight (dead end)
                        if abs(contour_angle) < self.dead_end_angle_threshold:
                            self.dead_end_detected = True
                            self.get_logger().warn(f'Dead end detected! Tape angle: {contour_angle:.1f}° (straight end)')
                    ...
                    if elapsed < self.backup_duration:
                        ...
                        # Backup complete - check if dead end detected
                        if self.dead_end_detected:
                            # Dead end: try probing forward instead of turning
                            self.recovery_phase = 'PROBE_FORWARD'
                            self.recovery_start_time = current_time
                            self.get_logger().warn('Dead end - starting PROBE_FORWARD to look for tape gap')
                        else:
                            # Normal case: move to right turn search
                            self.recovery_phase = 'TURN_RIGHT'
                            self.turn_accumulated = 0.0
                ...
                # Sub-phase 3b: Probe forward slowly looking for tape (for gaps/dead ends)
                    ...
                    if elapsed < self.probe_forward_duration:
                        twist = Twist()
                        twist.linear.x = self.forward_speed * self.probe_forward_speed  # Slow forward
        ...
        # Early dead end detection (while tape is still visible)
        cx, cy = contour_center
        early_dead_end = False
        if contour_angle is not None:
            # Check if tape is near bottom of frame and relatively straight
            tape_at_bottom = (cy > height * self.dead_end_y_threshold)
            tape_is_straight = (abs(contour_angle) < self.dead_end_angle_threshold)
            
            if tape_at_bottom and tape_is_straight:
                early_dead_end = True
                self.dead_end_detected = True
                self.cautious_frames += 1
                
                if self.cautious_frames == 1:  # First detection
                    self.get_logger().warn(f'Early dead end detected! Tape at bottom (y={cy}/{height}), angle={contour_angle:.1f}° - entering cautious mode')
            else:
                # Not a dead end anymore
                if self.cautious_frames > 0:
                    self.get_logger().info('Dead end cleared - resuming normal tracking')
                self.cautious_frames = 0
                self.dead_end_detected = False
        ...
        if self.sharp_turn_detected:
            ...
            # Early dead end handling: slow way down and prepare to probe forward
            speed = self.forward_speed * self.probe_forward_speed  # Slow to probing speed (30%)
            if self.cautious_frames % 30 == 1:  # Log occasionally
                self.get_logger().info(f'Cautious approach to dead end: speed={speed:.2f}, frames={self.cautious_frames}')
        ...
        # Adjust status for dead end
        tracking_status = 'CAUTIOUS' if early_dead_end else 'TRACKING'
        debug_img = self.create_debug_visualization(
            img, mask, best_contour, contour_center, contour_angle, 
            turn, lateral_error, p_term, i_term, d_term, process_time_ms, deadline_miss, tracking_status
        )
