...
class TapeRecoveryNode(Node):
    def __init__(self):
        ...
        # Subscribe to control commands (not needed for basic operation - disabled for now)
        # self.cmd_sub = self.create_subscription(Twist, '/cmd_vel', self.cmd_callback, 10)
        self.vel_pub = self.create_publisher(Twist, '/cmd_vel_recovery', 10)  # Separate topic to avoid circular sub
        self.last_cmd = None
        self.get_logger().info(f'TapeRecoveryNode started (motor_control={self.enable_motor_control})')
