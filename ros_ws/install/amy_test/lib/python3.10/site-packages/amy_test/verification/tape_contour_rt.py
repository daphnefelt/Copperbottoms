...
class TapeContourRT(Node):
    def __init__(self):
        ...
        # Gap coast override state
        self.gap_coast_override_active = False
        self.gap_coast_override_frames = 0
        ...
        self.angle_change_threshold = 50.0  # Increased from 35 - less sensitive, let LIDAR guide first
        ...
        self.turn_right_history = []  # Track recent turn_right values for right-turn bias
    ...
    def find_best_contour(self, mask):
        ...
        # Select best contour with continuity and center bias
        ...
        for contour in valid_contours:
            ...
            # Center bias: penalize distance from image center
            dx_center = cx - center_x
            dy_center = cy - center_y
            distance_from_center = np.sqrt(dx_center*dx_center + dy_center*dy_center)
            center_penalty = distance_from_center * 2.0  # Tune this weight as needed

            # If you want to keep trajectory continuity, you can add it here as well
            ...
            if self.last_contour_center is not None:
                ...
                continuity_penalty = distance_from_last * self.continuity_weight * 0.1  # Lower weight for continuity
            if self.last_contour_angle is not None and self.last_contour_center is not None:
                dx_last = cx - self.last_contour_center[0]
                dy_last = cy - self.last_contour_center[1]
                ...
                angle_penalty = angle_diff * self.angle_continuity_weight * 0.1  # Lower weight for angle

            # Combined score: area - penalties
            score = area - center_penalty - continuity_penalty - angle_penalty
            contour_scores.append((score, area, center_penalty, continuity_penalty, angle_penalty, cx, cy))
            if score > best_score:
                best_score = score
        ...
        if self.frame_count % 10 == 0 and len(contour_scores) > 1:
            self.get_logger().info(f'Multiple contours ({len(contour_scores)}): Evaluating center/continuity bias')
            for i, (score, area, center_pen, cont_pen, ang_pen, cx, cy) in enumerate(sorted(contour_scores, key=lambda x: x[0], reverse=True)[:3]):
                self.get_logger().info(f'  [{i}] score={score:.0f} (area={area:.0f}, center_pen={center_pen:.0f}, cont_pen={cont_pen:.0f}, ang_pen={ang_pen:.0f}) at ({cx},{cy})')
    ...
    def _turn_right_cb(self, msg: Bool):
        ...
        # Track history for right-turn bias logic
        self.turn_right_history.append(msg.data)
        if len(self.turn_right_history) > 10:  # Keep last 10 readings
            self.turn_right_history.pop(0)
    ...
    def image_callback(self, msg: Image):
        ...
        # BUT: Don't trigger if LIDAR is already providing right-turn guidance
        lidar_guiding = len(self.turn_right_history) >= 2 and all(self.turn_right_history[-2:])
        if contour_angle is not None:
            if self.last_angle is not None:
                angle_change = abs(contour_angle - self.last_angle)
                if angle_change > self.angle_change_threshold and not lidar_guiding:
        ...
        # GAP COAST OVERRIDE LOGIC
        # If tape is lost, turn_right contingency is DISABLED, and this is the first time in this tape loss event,
        # activate gap_coast_override for 5 frames, then disable until tape is reacquired.
        if not tape_detected:
            if not self.turn_right and not self.gap_coast_override_active and self.lost_tape_frames == 0:
                # Only activate ONCE per tape loss event
                self.gap_coast_override_active = True
                self.gap_coast_override_frames = 0
                self.get_logger().warn('GAP COAST OVERRIDE: Activated (will coast forward for 5 frames)')
        else:
            # Reset override when tape is reacquired
            self.gap_coast_override_active = False
            self.gap_coast_override_frames = 0
        ...
        if not tape_detected:
            ...
            # Reset sharp turn detection and last_angle to avoid false triggers on reacquisition
            self.sharp_turn_detected = False
            self.last_angle = None

            # GAP COAST OVERRIDE: If active, coast forward for 5 frames, then disable
            if self.gap_coast_override_active and self.gap_coast_override_frames < 5:
                twist = Twist()
                twist.linear.x = self.forward_speed
                twist.angular.z = 0.0  # Always drive straight during gap coast override
                if self.enable_motor_control:
                    self.vel_pub.publish(twist)
                self.gap_coast_override_frames += 1
                self.get_logger().info(f'GAP COAST OVERRIDE: Coasting forward for   (frame {self.gap_coast_override_frames}/8)')
                ...
                debug_img = self.create_debug_visualization(
                    ...
                    twist.angular.z, 0.0, 0.0, 0.0, 0.0, process_time_ms, False,
                    f'GAP_COAST_OVERRIDE ({self.gap_coast_override_frames}/5)'
                )
                self.publish_debug_image(debug_img, msg.header)
                # If finished, disable override
                if self.gap_coast_override_frames >= 8:
                    self.gap_coast_override_active = False
                return
            # ...existing code for gap coasting, coasting, search, and recovery...
            ...
            # Phase 3: Advanced recovery (backup, then systematic turn search)
                ...
                if self.recovery_phase == 'BACKING_UP':
                    ...
                    if best_contour is not None and contour_center is not None:
                        # Tape found! Now turn in place to align tape to center
                        cx = contour_center[0]
                        center_x = width / 2
                        error = (cx - center_x) / center_x
                        align_threshold = 0.05  # Acceptable alignment error
                        if abs(error) > align_threshold:
                            # Turn in place to center tape
                            twist = Twist()
                            twist.linear.x = 0.0
                            twist.angular.z = -np.sign(error) * self.max_turn * 0.5
                            if self.enable_motor_control:
                                self.vel_pub.publish(twist)
                            status_text = f'ALIGNING_TAPE (error={error:.2f})'
                            end_time = time.perf_counter()
                            process_time_ms = (end_time - start_time) * 1000.0
                            self.update_statistics(process_time_ms)
                            debug_img = self.create_debug_visualization(
                                img, mask, best_contour, contour_center, contour_angle,
                                twist.angular.z, error, 0.0, 0.0, 0.0, process_time_ms, False, status_text
                            )
                            self.publish_debug_image(debug_img, msg.header)
                            return
                        else:
                            # Tape is centered, exit recovery
                            self.get_logger().info('Tape centered after backup and alignment - resuming tracking')
                    ...
                    elif elapsed < self.backup_duration:
                        # Continue backing up
                        twist = Twist()
                        twist.linear.x = -max(0.15, self.forward_speed * 0.5)
                        twist.angular.z = 0.0
                        if self.enable_motor_control:
                            self.vel_pub.publish(twist)
                ...
                # Sub-phase 3e: Final stop (or contingency mode)
                    ...
                    if self.turn_right:
                        ...
                        # Coast forward, use last known tape center if available
                        twist.linear.x = self.forward_speed
                        if self.last_contour_center is not None:
                            cx = self.last_contour_center[0]
                            center_x = width / 2
                            error = (cx - center_x) / center_x
                            ...
                            twist.angular.z = 0.0
                        status_text = 'CONTINGENCY: COAST_FORWARD_CENTER_TAPE'
                    if self.enable_motor_control:
                        self.vel_pub.publish(twist)
                    end_time = time.perf_counter()
                    process_time_ms = (end_time - start_time) * 1000.0
        ...
        # LIDAR-guided right turn bias: If LIDAR detects right opening for 2 consecutive readings,
        # add a right-turn bias to help robot follow tape around right turns
        # This prevents missing right turns and continuing straight
        if len(self.turn_right_history) >= 2 and all(self.turn_right_history[-2:]):
            # 2 consecutive turn_right=True readings detected
            right_turn_bias = 0.3  # Add 0.3 rad/s right turn bias
            turn += right_turn_bias
            turn = float(np.clip(turn, -self.max_turn, self.max_turn))
            if self.frame_count % 10 == 0:  # Log every 10 frames for better visibility
                self.get_logger().info(f'LIDAR right opening detected - adding right turn bias: {right_turn_bias:.2f}')
