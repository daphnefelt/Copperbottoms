...
class RoverControl(Node):
    def __init__(self):
        ...
        # MAVLink connection
        self.connection = mavutil.mavlink_connection('/dev/ttyACM1', baud=921600)
        ...
        self.commands_received = False  # Don't send commands until we get input
    ...
    def cmd_vel_callback(self, msg):
        ...
        self.commands_received = True  # Now we can start sending commands
    ...
    def send_continuous_commands(self):
        ...
        # Don't send commands until we receive at least one input
        ...
        try:
            ...
            steering_scaled = int(1500 + (self.last_steering * 300))   # Reduced steering range for better control
            
            # Clamp to safe PWM ranges
            throttle_scaled = max(1400, min(1600, throttle_scaled))  # Max speed 1600
            steering_scaled = max(1200, min(1800, steering_scaled))   # Reduced steering range
