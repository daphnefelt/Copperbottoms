#!/usr/bin/env python3
"""
ROS2 ArduPilot Rover Node - Modified for Slower, Safer Operation
...
class ArduPilotRoverNode(Node):
    def __init__(self):
        ...
        # Get parameters - check for 'device' first (used in ros2 run command)
        device_param = self.get_parameter('device').value
        connection_param = self.get_parameter('connection_string').value
        
        # Use device parameter if it's not the default, otherwise use connection_string
        if device_param != '/dev/ttyACM0':
            self.connection_string = device_param
        else:
            self.connection_string = connection_param
        ...
        self.get_logger().info('Initializing ArduPilot Rover Node - Slow & Safe Version...')
    ...
    def cmd_vel_callback(self, msg):
        ...
        # Moderate throttle multiplier for reasonable speed
        throttle_raw = msg.linear.x * -300  # Increased from -200, but less than original -400
        
        # Apply offset for non-zero commands to overcome motor deadband
        if abs(throttle_raw) > 5:  # Only if there's actual input
            if throttle_raw >= 0:
                throttle_with_offset = throttle_raw + 50  # Increased from 20, but less than original 80
            else:
                throttle_with_offset = throttle_raw - 50  # Increased from 20, but less than original 80
        else:
            throttle_with_offset = 0  # No movement when no input

        # Scale to MAVLink range with moderate limits
        self.current_throttle = int(np.clip(throttle_with_offset, -250, 250))  # Increased from -150, but less than original -300

        # Reduced steering multiplier for better control
        self.current_steering = int(np.clip(msg.angular.z * 300, -600, 600))  # Keep steering as is
