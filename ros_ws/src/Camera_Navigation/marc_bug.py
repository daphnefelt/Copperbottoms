...
class Bug1(Node):
    ...
    def __init__(self):
        ...
        self._right_samples  = deque()
        self._prev_avg_right = float('inf')
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if self.mode == self.MODE_BACKING_UP:
            ...
            if now - self.mode_start_time < self.backup_time:
                self._publish(-self.backup_speed, 0.0) # self._turn_dir * self.turn_speed
        ...
        if self.mode == self.MODE_FIND_WALL:
            # buffer readings from the last 0.5s
            self._right_samples.append((now, right_dist))
            while self._right_samples and now - self._right_samples[0][0] > 0.5:
                self._right_samples.popleft()

            avg_right = sum(d for _, d in self._right_samples) / len(self._right_samples)

            # detect divot: right dist jumped OUTWARD by >= 1m vs prior 0.5s avg
            # (wall was there, now suddenly it's not — a gap/divot)
            if avg_right - self._prev_avg_right >= 1.0 and len(self._right_samples) >= 3:
                self._coast_until = now + 4.0
                self.get_logger().info(
                    f'[FIND_WALL] divot detected: avg {self._prev_avg_right:.2f} -> {avg_right:.2f}m, coasting 4s')
            self._prev_avg_right = avg_right

            if now < self._coast_until:
                self._publish(self.forward_speed, 0.0)  # coast straight
                return
