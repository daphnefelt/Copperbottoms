...
class TapeContourRT(Node):
    def __init__(self):
        ...
        self.turn_right_history = []  # Track recent turn_right values
        self.turn_right_history_size = 3  # Need 3 consecutive "enabled" to exit coast
    ...
    def _turn_right_cb(self, msg: Bool):
        ...
        # Track history for coast mode exit logic
        self.turn_right_history.append(msg.data)
        if len(self.turn_right_history) > self.turn_right_history_size:
            self.turn_right_history.pop(0)  # Keep only last 3
    ...
    def image_callback(self, msg: Image):
        ...
        if not tape_detected:
            ...
            # Phase 1: Coast mode - NEW LOGIC based on /turn_right topic
            # Stay in coast mode INDEFINITELY if /turn_right is "disabled"
            # Exit to search mode only if 3 consecutive "enabled" messages received
            
            # Check if we should stay in coast mode
            should_coast = False
            coast_reason = ""
            
            if not self.turn_right:
                # /turn_right is currently disabled - stay in coast mode indefinitely
                should_coast = True
                coast_reason = "turn_right=DISABLED"
            elif len(self.turn_right_history) < self.turn_right_history_size:
                # Don't have enough history yet - stay in coast mode
                should_coast = True
                coast_reason = f"building history ({len(self.turn_right_history)}/{self.turn_right_history_size})"
            elif not all(self.turn_right_history):
                # Not all recent messages were "enabled" - stay in coast mode
                enabled_count = sum(self.turn_right_history)
                should_coast = True
                coast_reason = f"waiting for 3x enabled ({enabled_count}/3)"
            else:
                # All 3 recent messages were "enabled" - exit coast mode to search
                should_coast = False
                coast_reason = "3x enabled detected"
            ...
            if should_coast:
                if self.last_contour_center is not None:
                    ...
                    twist = Twist()
                    twist.linear.x = max(0.2, self.forward_speed * 0.7)
                    twist.angular.z = turn
                    if self.enable_motor_control:
                        self.vel_pub.publish(twist)
                    
                    if self.lost_tape_frames == 1 or self.lost_tape_frames % 30 == 0:
                        self.get_logger().warn(
                            f'Tape lost - coasting indefinitely (frame {self.lost_tape_frames}, {coast_reason})'
                        )
                    ...
                    process_time_ms = (end_time - start_time) * 1000.0
                    self.update_statistics(process_time_ms)
                    status_text = f'COASTING ({coast_reason})'
                    debug_img = self.create_debug_visualization(
                        img, mask, None, None, None,
                        turn, lateral_error, 0.0, 0.0, 0.0, process_time_ms, False, status_text
                    )
                    self.publish_debug_image(debug_img, msg.header)
                    return
            
            # Phase 2: Active search mode - triggered by 3 consecutive "enabled" on /turn_right
            # Search is BIASED to turn right
            elif self.lost_tape_frames <= self.max_coast_frames + self.max_search_frames:
                if not self.search_mode:
                    self.reset_pid_state()
                    self.search_mode = True
                    self.get_logger().warn(f'Entering search mode - {coast_reason}, biasing RIGHT turn')
                
                # BIAS: Always prefer right turn in search mode
                if self.sharp_turn_detected:
                    search_turn = 1.0  # Maximum right turn
                else:
                    search_turn = 0.6  # Biased to turn right
                ...
                process_time_ms = (end_time - start_time) * 1000.0
                self.update_statistics(process_time_ms)
                search_frames = self.lost_tape_frames - self.max_coast_frames
                status_text = f'SEARCHING RIGHT ({search_frames}/{self.max_search_frames})'
                debug_img = self.create_debug_visualization(
                    img, mask, None, None, None,
                    search_turn, 0.0, 0.0, 0.0, 0.0, process_time_ms, False, status_text
                )
                self.publish_debug_image(debug_img, msg.header)
                return
