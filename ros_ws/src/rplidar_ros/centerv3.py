...
class HallwayCenterNode(Node):
    ...
    def __init__(self):
        ...
        self.filter_alpha    = 0.3   # low-pass on corrected_error before derivative
        ...
        self.MODE_HALLWAY_TURN_STRAIGHTEN  = 'HALLWAY_TURN_STRAIGHTEN'
        self.mode            = self.MODE_CENTERING
        self.mode_start_time = 0.0
        self.turn_dir        = 1.0   # +1 = left, -1 = right
        self.suppress_right_turn  = False
        self.reacquire_right_dis  = 4.0   # right_dist threshold (m) to end straighten
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if self.mode == self.MODE_HALLWAY_TURN_RIGHT:
            ...
            # Turn complete -> enter straighten state, drive straight until
            # right wall is re-acquired.
            straight = Twist()
            straight.linear.x = self.forward_speed
            self.vel_pub.publish(straight)
            self.suppress_right_turn = True
            self._enter_mode(self.MODE_HALLWAY_TURN_STRAIGHTEN)
            self.get_logger().info('Hallway turn complete. Straightening until right wall re-acquired.')
            return

        # ------------------------------------------------------------------
        # HALLWAY_TURN_STRAIGHTEN: drive forward with normal centering but
        # suppress the right-turn trigger until the right wall is re-acquired
        # (right_dist < self.reacquire_right_dis). Front obstacle still preempts.
        # ------------------------------------------------------------------
        if self.mode == self.MODE_HALLWAY_TURN_STRAIGHTEN:
            right_dist_check = self._valid_median(
                ranges, msg, -math.pi / 2, self.side_half_cone)
            if not math.isnan(right_dist_check) and right_dist_check < self.reacquire_right_dis:
                self.suppress_right_turn = False
                self._enter_mode(self.MODE_CENTERING)
                self.get_logger().info(
                    f'Right wall re-acquired ({right_dist_check:.2f} m). Resuming centering.')
                # fall through to CENTERING this cycle
        ...
        # -- Right-opening detection: trigger a 90° right turn when the
        # right cone returns more than 5 inf readings (no wall within lidar range).
        s_r, e_r = self._cone_indices(msg, -math.pi / 2, self.side_half_cone)
        right_cone = ranges[s_r:e_r + 1]
        right_inf_count = int(np.sum(np.isinf(right_cone)))
        if not self.suppress_right_turn and right_inf_count > 5:
            self.get_logger().info(
                f'Right opening detected ({right_inf_count} inf readings). Turning right.')
