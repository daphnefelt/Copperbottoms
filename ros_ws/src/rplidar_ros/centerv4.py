...
class HallwayCenterNode(Node):
    ...
    def __init__(self):
        ...
        self.max_turn               = 0.75    # clip for servo angular output
        self.turn_target_dist       = 1.8    # fallback target if snapshot is bad (m)
        self.turn_trigger_inf_count = 10      # inf beams in right cone to trigger
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        # the right cone has more than N inf readings. Snapshot the buffered
        # dist_at_90 as the servo target, or fall back to turn_target_dist.
        ...
        if (not self.suppress_right_turn
                and right_inf_count > self.turn_trigger_inf_count):
            ...
            self.get_logger().info(
                f'Right opening detected ({right_inf_count} inf readings). '
