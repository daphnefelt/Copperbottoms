...
class HallwayCenterNode(Node):
    ...
    def __init__(self):
        ...
        self.backup_time     = 1.5   # seconds to reverse after a hit
        ...
        self.turn_target_dist       = 1.5    # fallback target if snapshot is bad (m)
        ...
        # -- Right-wall sustained-crossing counters --------------------------
        # Only fire a right-branch correction after this many consecutive
        # scans past the bumper/boundary. Filters out cutouts and spurious
        # spikes up to ~2 seconds (at 10 Hz).
        self.right_sustain_count         = 20
        self.right_over_boundary_counter = 0
        self.right_under_bumper_counter  = 0
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if math.isnan(right_dist):
            # Right wall not visible -- fall back to left or straight
            self.right_over_boundary_counter = 0
            self.right_under_bumper_counter  = 0
            if math.isnan(left_dist):
                corrected_error = 0.0
                branch = 'none'
            else:
            ...
            # Right wall visible -- primary mode. Update sustained-crossing
            # counters; correction only fires after N consecutive scans past
            # the bumper or boundary.
            if right_dist > right_boundary:
                self.right_over_boundary_counter += 1
                self.right_under_bumper_counter = 0
            elif right_dist < right_bumper:
                self.right_under_bumper_counter += 1
                self.right_over_boundary_counter = 0
            else:
                self.right_over_boundary_counter = 0
                self.right_under_bumper_counter = 0

            if self.right_over_boundary_counter >= self.right_sustain_count:
                corrected_error = -(right_dist - right_boundary)
            elif self.right_under_bumper_counter >= self.right_sustain_count:
        ...
        self.get_logger().info(
            ...
            f'(err={corrected_error:+.2f} filt={self.filtered_error:+.2f}) '
            f'ovr={self.right_over_boundary_counter} '
            f'und={self.right_under_bumper_counter} | {direction}',
            throttle_duration_sec=0.5)
