...
class HallwayCenterNode(Node):
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        goal_pt = np.array([shortest_dist_right*np.cos(towards_right), shortest_dist_left*np.sin(towards_right)]) + 
        2.5*np.array([np.cos(towards_right+np.pi/2), np.sin(towards_right+np.pi/2)])
        if goal_pt[0] > 0:
            dir = 0.3
