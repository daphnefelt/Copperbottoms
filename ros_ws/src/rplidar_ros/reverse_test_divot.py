...
class LidarDebugNode(Node):
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if self.mode == self.MODE_STRAIGHT:
            ...
            twist.angular.z = self.PD_steering(angle_error, 0.0)
            self.vel_pub.publish(twist)
        
            """ if divot, stay steady on any parallel reading, coast if no parallel reading, exit condition is inlet
            timeout on not seeing inlet - return to straight
            turn and obstacle must override  ????
            """
